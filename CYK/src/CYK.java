...
public class CYK{
	...
	void calcCYK(int [] w)
	{
		...
		//Fill in your program here
                int i, j;
                int first_var, second_var, head;
                // Base case
                for (i = 0; i < L; i++)
                    for (j = 0; j < VarNum; j++)
                        X[i][i][j] = existProd(j, w[i], -1);
                int  stride, first_index, k, second_index;
		// Induction
                for (stride = 1; stride < L; stride++) // stride
                {
                    for (first_index = 0; first_index < L - stride; first_index++) // first index
                    {
                        second_index = first_index + stride;
                        for (k = first_index; k < second_index; k++)
                        {
                            for (first_var = 0; first_var < VarNum; first_var++)
                            {
                                if (X[first_index][k][first_var] == false) continue;
                                for (second_var = 0; second_var < VarNum; second_var++)
                                {
                                    if (X[k + 1][second_index][second_var])
                                    {
                                        for (head = 0; head < VarNum; head++)
                                        {
                                            if (X[first_index][second_index][head]) continue;
                                            if (existProd(head, first_var, second_var))
                                            {
                                                X[first_index][second_index][head] = true;
                                                break;
                                            }
                                        }
                                    }
