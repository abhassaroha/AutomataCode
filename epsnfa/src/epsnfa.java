...
public class epsnfa{
	...
	int[] union(int s1,int t1,int s2,int t2)
	{
		int [] st=new int[2];
                ...
                st[1] = incCapacity();
                // e to start states
                g[st[0]][epssymbol][s1] = true;
                g[st[0]][epssymbol][s2] = true;
                // e from end states
                g[t1][epssymbol][st[1]] = true;
                g[t2][epssymbol][st[1]] = true;
	...
	int[] concat(int s1,int t1,int s2,int t2)
	{
		int [] st=new int[2];
                ...
                st[0] = s1;
                st[1] = t2;
                // feed string accepted by nfa1 to nfa2's start state on e
		g[t1][epssymbol][s2] = true;
		return st;
	}
